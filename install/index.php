...
if($method == 'show_license') {
	...
	$default_configfile = './install/include/install_var.php';
	...
	if($submit) {
		...
		save_vpnconfig_file(ROOT_PATH.'./data/config.inc.php', $dbhost, $dbname,$dbpw,$dbuser);
		...
		runquery($sql);
		$db->query("INSERT INTO user (username, password, active, creation,quota_cycle, quota_bytes,  email,coin) VALUES ('$username',PASSWORD('$password'),'1',DATE_FORMAT(NOW() ,'%Y-%m-01'),DAY(LAST_DAY(now())),'10737418240','$email','1000')");
		$db->query("INSERT INTO admin (username) VALUES ('$username')");
	...
	if(VIEW_OFF) {
		...
		echo 'VPN Admin 1.0 安装成功<br>';
		echo '<a href="../">马上进入首页</a>';
		echo '</div>';
		show_footer();
	}
